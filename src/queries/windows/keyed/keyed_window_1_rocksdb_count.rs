                ...
                input.for_each(|time, data| {
                    ...
                    if last_slide_seen < current_slide {
                        ...
                        for sl in (start..end).step_by(window_slide_ns) {
                            ...
                            window_contents.insert(((sl - window_slide_ns).to_be(), MIN_KEY.to_be()), 0);  // Start timestamp of window, minimum possible key
                            // TODO (john): Omit adding the end here and change loop condition below
                            // println!("Inserting dummy record:: time: {:?}, value:{:?}", window_end, 0);
                            window_contents.insert((window_end.to_be(), MAX_KEY.to_be()), 0);  // End timestamp of window, max possible key
                        }
                    ...
                    for record in buffer.iter() {
                        ...
                        let key = ((record.1).to_be(), auction_id.to_be() as u64); // Event time, auction id
                        // Add record
